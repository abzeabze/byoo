...
async function createFile(fileName, combinedMetadata, context) {
     ...
     * @param combinedMetadata
     */
    function objectTabs(wb, combinedMetadata) {
        combinedMetadata.forEach(element => {
            var ws = wb.addWorksheet(element.name);
            let headers = ['Label', 'Name', 'Help Text', 'Is Standard', 'Formula', 'Max Length', 'Type', 'Is unique', 'precision', 'Scale', 'Encrypted', 'ExternalId', 'PicklistValues',
     ...
     */
    function simplePicklistWorkSheet(wb, combinedMetadata, context) {
        var ws_Picklist = wb.addWorksheet("Picklist");
        let headers = ['Object Name', 'Field Name', 'Value', 'Is Active'];
        ...
        combinedMetadata.forEach(element => {
            ...
            element.fields.forEach(field => {
                let initialRowNum_field = rowNum;
                let totalRows_field = -1;
                if (field.type == 'picklist') {
                    ...
                    field.picklistValues.forEach(option => {
                        if (!field.controllerName) {
                            let excelColumns = [objName, field.name, option.value, option.active ? "Yes" : "No"];
                    ...
                    let finalNum = initialRowNum_field + totalRows_field;
     ...
     * @param context Salesforce CLI object context
     */
    function multiPicklistWorkSheet(wb, combinedMetadata, context) {
        var ws_Picklist = wb.addWorksheet("Dependent Picklist");
        let headers = ['Object Name', 'Controlling Field Name', 'Dependent Field Name', 'Controlling Value', 'Value', 'Value Active', 'Is Multi Picklist'];
        addHeader(ws_Picklist, headers, 1, headerStyle);
        //header was row number 1
        var rowNum = 2;
        var objectColumnNumber = 1;
        let prevControllingField = null;
        let initialRowNum_cntrl_field = rowNum;
        let totalRows_cntrl_field = -1;
        var controlingFieldColumnNumber = 2;
        let prevDependentField = null;
        let initialRowNum_Dependent_field = rowNum;
        let totalRows_Dependent_field = -1;
        var depdepndentFieldColumnNumber = 3;
        let prevControllingValue = null;
        let initialRowNum_cntrl_Value = rowNum;
        let totalRows_cntrl_Value = -1;
        ...
        combinedMetadata.forEach(element => {
            ...
            lstMultiPicklistFields.forEach(field => {
                ...
                if (field.type == 'picklist') {
                    ...
                    sortedList.forEach(option => {
                        ...
                        let excelColumns = [objName, field.controllerName, field.name, option.validFor, option.value, option.active ? "Yes" : "No", isMultiPicklist];
                        addPickListToExcel(ws_Picklist, rowNum, excelColumns);
                        //Logic to Merge Controlling Field Column 2
                        if (prevControllingField != field.controllerName) {
                            let finalControllingFieldCount = initialRowNum_cntrl_field + totalRows_cntrl_field;
                        ...
                        if (prevDependentField != field.name) {
                            let finalDependentFieldCount = initialRowNum_Dependent_field + totalRows_Dependent_field;
                        ...
                        if (prevControllingValue != option.validFor) {
                            let finalControllingValueCount = initialRowNum_cntrl_Value + totalRows_cntrl_Value;
            ...
            let finalObjectRowCount = initialRowNum_obj + totalRows_obj;
